...
**Amendments:**
- April 8, 2026 — Sidecar metadata location integrated into the main body; added folder ID caching, init overwrite behavior, and `.wkignore` support.
- April 2026 — Partially superseded by ADR-007: single-entry init flags (`--server-path`/`--local-path`) removed, `--overwrite` now replaces `[[sync]]` entries, incremental sync-entry management moved to the `wk sync` group. See note below.
...
- **`profiles.env` placement.** Lives at `<project-root>/profiles.env` — project root, **outside** `.wk/` (ADR-006 Sub-decision 3, April 20, 2026 revision). An earlier revision moved the file *into* `.wk/` to inherit the self-gitignore, but that placement created a bootstrap contradiction: `.wk/` is CLI-created by `wk init`, and `profiles.env` is developer-authored and must exist *before* init can resolve the profile it names. Issue #40 made the contradiction concrete. The file is now at project root; `wk init` idempotently appends `profiles.env` to `<project-root>/.gitignore` as the safety net that `.wk/.gitignore` used to provide. Principle: developer-authored / read-only-to-CLI files don't belong inside the CLI-managed directory, regardless of gitignore mechanics.
